...
class SessionManager {
  ...
  addUserInput(text, source = 'chat') {
    const normalizedText = typeof text === 'string' ? text : String(text?.text || text || '');
  ...
  generateContextSummary(action, details) {
    ...
    const content = typeof details.content === 'string'
      ? details.content
      : String(details.content?.text || details.content || '');
    
    switch (action) {
      case 'speech_transcription':
        return `User spoke: "${content.substring(0, 50)}..." (${skill} mode)`;
      case 'chat_input':
        return `User typed: "${content.substring(0, 50)}..." (${skill} mode)`;
      case 'llm_response':
      ...
      case 'user_message':
        return `User: "${content.substring(0, 50)}..." (${skill})`;
      case 'model_response':
